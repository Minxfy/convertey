...
export class PdfService {
  ...
  async pdfToJpg(pdfBuffer: Buffer): Promise<Buffer> {
    try {
      ...
      try {
        ...
        await execAsync(`pdftoppm -f 1 -l 1 -jpeg -r 300 "${pdfPath}" "${outputPath.replace('.jpg', '')}"`);
        ...
        console.warn('Poppler conversion failed, trying alternative method:', popplerError.message);
  ...
  async pdfToPptx(pdfBuffer: Buffer): Promise<Buffer> {
    try {
      ...
      slide.addText('Note: This is a simplified conversion. For better results, consider using specialized PDF processing tools.', { 
        x: 1, 
        y: 4, 
        fontSize: 12, 
        color: '999999' 
